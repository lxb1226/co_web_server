...
bool HttpRequest::parse(Buffer &buff) {
  ...
  while (buff.getReadableBytes() && state_ != FINISH) {
    //        const char *lineEnd = search(buff.peek(), buff.beginWrite(), CRLF,
    //        CRLF + 2);
    const char *lineEnd =
        search(buff.peek(), static_cast<const char *>(buff.beginWrite()), CRLF,
               CRLF + 2);
  ...
  //    version_.c_str()); DEBUG("[%s], [%s], [%s]", method_.c_str(),
  //    path_.c_str(), version_.c_str()) DEBUG(method_.c_str(), " , ",
  //    path_.c_str(), " , ", version_.c_str());
...
void HttpRequest::ParseBody_(const string &line) {
  ...
  DEBUG("Body:{}, len:{}", line.c_str(), line.size())
}
...
// bool HttpRequest::UserVerify(const string &name, const string &pwd, bool
// isLogin) {
...
//     /* 查询用户及密码 */
//     snprintf(order, 256, "SELECT username, password FROM user WHERE
//     username='%s' LIMIT 1", name.c_str()); LOG_DEBUG("%s", order);
...
//         VALUES('%s','%s')", name.c_str(), pwd.c_str()); LOG_DEBUG( "%s",
//         order); if(mysql_query(sql, order)) {
